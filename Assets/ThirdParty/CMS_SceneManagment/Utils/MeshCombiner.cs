...
using BoneTransformDict = System.Collections.Generic.Dictionary<string, System.Tuple<UnityEngine.Transform, string>>;
using System;
		...
		*/
		...
		static void AddParent(string BoneName, HierarchyDict BoneHierarchy, BoneTransformDict AllBones, GameObject DummyParent)
		{
			...
			if (AllBones.ContainsKey(BoneName))
			{
				...
				if (!BoneHierarchy.ContainsKey(bone_tuple.Item2))
				{
					AddParent(bone_tuple.Item2, BoneHierarchy, AllBones, DummyParent);
					// Unparent all children of parents
					Unparent(BoneHierarchy[bone_tuple.Item2], DummyParent);
				}


				bone_tuple.Item1.parent = BoneHierarchy[bone_tuple.Item2];
				actual_bone = bone_tuple.Item1;
			}
